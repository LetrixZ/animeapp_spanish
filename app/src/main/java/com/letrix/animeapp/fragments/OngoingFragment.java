...
public class OngoingFragment extends Fragment implements TitlelessAdapter.OnItemClickListener {
    ...
    private void initRecycler() {
        ...
        mViewModel.getOngoingsList(1).observe(getViewLifecycleOwner(), new Observer<ArrayList<AnimeModel>>() {
            @Override
            public void onChanged(ArrayList<AnimeModel> animeModels) {
                adapter = new TitlelessAdapter(animeModels, OngoingFragment.this::onItemClick);
                mRecyclerView.setAdapter(adapter);
                progressBar.setVisibility(View.GONE);

                mViewModel.getOngoingsList(2).observe(getViewLifecycleOwner(), new Observer<ArrayList<AnimeModel>>() {
                    @Override
                    public void onChanged(ArrayList<AnimeModel> animeModels) {
    ...
    public void onItemClick(int position) {
        mViewModel.setSelectedAnime(mViewModel.getOngoingsList(1).getValue().get(position));
